...
By default, plugins are not enabled for operations. You can enable one or more plugins for an operation using the [`plugins`](project:/pages/reference/guildfile.md#operation-plugins) operation attribute.
...
> <span data-guild-class="callout important">Important</span> Summary plugins are not supported with Guild [output scalars](project:/pages/docs/scalars.md#output-scalar). You must explicitly log scalars as TensorBoard summaries from the operation script itself using one of the [supported methods](project:/pages/docs/scalars.md#tensorboard-summaries).
