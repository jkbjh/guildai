...
You can modify the way Guild logs output scalars using the `output-scalars` operation attribute in a [Guild file](project:/pages/docs/guildfiles.md).
...
Refer to [*Guild File Reference*](project:/pages/reference/guildfile.md#output-scalars) for a specification of the `output-scalars` attribute.

Refer to [*Guild File Cheatsheet*](project:/pages/cheatsheets/.md#output-scalars) for examples of `output-scalars`.
...
| [PyTorch API](https://pytorch.orgproject:/pages/docs/stable/tensorboard.html.md) | Operation uses PyTorch |
...
Show runs scalars using [`guild runs info`](project:/pages/commands/runs-info.md).
...
To show all scalars for multiple runs, use the `--print-scalars` option with [`guild compare`](project:/pages/commands/compare.md).
