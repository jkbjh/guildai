...
Operations are defined in [Guild files](/pages/docs/guildfiles).

Operations are defined using one of two formats:

- [Operation only](/pages/reference/guildfile#operation-only-format)
- [Full format](/pages/reference/guildfile#full-format)
...
Refer to [*Guild File Reference*](/pages/reference/guildfile#guild-file-format) for more information on file format.
...
> <span data-guild-class="callout tip">Tip</span> The [`guild ops`](/pages/commands/ops) command is a shortened alias for `operations`.</span>
...
By default, Guild passes additional arguments to the `main` module that correspond to operation [flag values](/pages/docs/flags). You can, however, specify arguments in the `main` attribute. These additional arguments are provided regardless of the flag-specific arguments.
...
The script `train.r` is prefixed with `.guild/sourcecode/` because operations run in the context of a [*run directory*](/pages/docs/runs#run-dir), not the project directory. Guild copies source code files from the project to the run directory under `.guild/sourcecode` by default. See [Operation Source Code](#operation-source-code) for more information.
...
> <span data-guild-class="callout tip">Tip</span> Use the `--print-cmd` option with [`guild run`](/pages/commands/run) to show the command Guild uses to start an operation.
...
See [*Guild File Reference*](/pages/reference/guildfile#source-code) for information on configuring rules for source code copies.

See [*Guild File Cheatsheet*](/pages/cheatsheets/guildfile#source-code) for examples.
