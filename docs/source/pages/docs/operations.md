...
Guild uses operations to start [*runs*](project:/pages/docs/runs.md). Specify an operation for [`guild run`](project:/pages/commands/run.md) using the format:

``` bash
guild run OPERATION
```

Operations are defined in [Guild files](project:/pages/docs/guildfiles.md).

Operations are defined using one of two formats:

- [Operation only](project:/pages/reference/guildfile.md#operation-only-format)
- [Full format](project:/pages/reference/guildfile.md#full-format)
...
Refer to [*Guild File Reference*](project:/pages/reference/guildfile.md#guild-file-format) for more information on file format.

List available operations for a project using [`guild operations`](project:/pages/commands/operations.md):

``` bash
guild operations
```

> <span data-guild-class="callout tip">Tip</span> The [`guild ops`](project:/pages/commands/ops.md) command is a shortened alias for `operations`.</span>
...
By default, Guild passes additional arguments to the `main` module that correspond to operation [flag values](project:/pages/docs/flags.md). You can, however, specify arguments in the `main` attribute. These additional arguments are provided regardless of the flag-specific arguments.
...
The script `train.r` is prefixed with `.guild/sourcecode/` because operations run in the context of a [*run directory*](project:/pages/docs/runs.md#run-dir), not the project directory. Guild copies source code files from the project to the run directory under `.guild/sourcecode` by default. See [Operation Source Code](#operation-source-code) for more information.
...
> <span data-guild-class="callout tip">Tip</span> Use the `--print-cmd` option with [`guild run`](project:/pages/commands/run.md) to show the command Guild uses to start an operation.
...
See [*Guild File Reference*](project:/pages/reference/guildfile.md#source-code) for information on configuring rules for source code copies.

See [*Guild File Cheatsheet*](project:/pages/cheatsheets/guildfile.md#source-code) for examples.
