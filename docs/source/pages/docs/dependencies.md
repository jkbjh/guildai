...
The [`requires`](project:/pages/reference/guildfile.md#operation-requires) operation resource is a list of either *inline* or *named* resources.
...
Named resoures are defined in a model [`resources`](project:/pages/reference/guildfile.md#model-resources) attribute. Named resources are specified as strings in `requires` with operation name.
...
Guild supports a number of resource types, or *resource source* types, which are described below. Refer to [*Guild File Reference*](project:/pages/reference/guildfile.md#resource-sources) for a specification of each type.
...
Guild either links to the file or creates a copy based on the source [`target-type`](project:/pages/reference/guildfile.md#resource-source-target-type) attribute. By default Guild creates links to resolved files. To create a copy, specify `copy` for `target-type`:
...
For additional attributes used to configure sources, see [*Guild File Reference*](project:/pages/reference/guildfile.md#resource-source-attributes).
...
To ensure that a downloaded source is not corrupt, use the [`sha256`](project:/pages/reference/guildfile.md#resource-source-sha256) attribute to define a SHA-256 digest. Guild checks the digest of downloaded files and stops with an error message if it doesn't match the specified value. Use [`guid download`](project:/pages/commands/download.md) to pre-fetch a network file and calculate its SHA-256 digest for use with this attribute.

Refer to see [*Guild File Reference*](project:/pages/reference/guildfile.md#resource-source-attributes) for other attributes used to configure network file sources.

### Run Files

To use files generated by other operations for a run, use an `operation` source.

The following defines three operations that constitute a [pipeline](project:/pages/docs/pipelines.md) for preparing data, training, and testing:
...
To select a subset of run files, use the [`select`](project:/pages/reference/guildfile.md#resource-source-select) source attribute. For example, to select only files ending with `.hdf5` (a common extension for serialized Keras files):
...
Other [source attributes](project:/pages/reference/guildfile.md#resource-source-attributes) may be used to further configure operation sources.
...
> <span data-guild-class="callout note">Note</span> Guild uses the flag name when writing values to configuration files. To write to nested values, use dots in the flag names to denote nesting levels. A future version of Guild will support configuration files using the [flags interface](project:/pages/docs/flags.md#flags-interface), which will provide more flexibility and features to support this case.
...
Use the [`help`](project:/pages/reference/guildfile.md#resource-source-help) attribute to provide a user friendly message when the check fails.
