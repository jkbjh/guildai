...
Flags are specified for an operation using `NAME=VALUE` arguments to [`guild run`](project:/pages/commands/run.md).
...
Guild works across platforms and languages using standard interfaces when possible. Guild does not require changes to script code to support Guild-specific configuration. Guild uses script inspection and explicit configuration in [Guild files](project:/pages/docs/guildfiles.md) for flag information.

When Guild does not have explicit configuration (e.g. when a script is run directly) it attempts to infer the flags interface by inspecting the script. See [*Default Behavior*](project:/pages/docs/defaults.md#flags-interface) for more information.

The flags interface is configured for an operation using the `flags-dest` attribute. See [`flags-dest` in *Guild File Reference*](project:/pages/reference/guildfile.md#operation-flags-dest) for configuration details.

For example of different interfaces, see [*Guild File Cheatsheet*](project:/pages/cheatsheets/guildfile.md#flags-interface-python-modules).
...
<pre>`--<em>FLAG_ARG_NAME</em> <em>ENCODED_FLAG_VALUE</em>`{em-code}</pre>
...
> <span data-guild-class="callout tip">Tip</span> Use the `--print-cmd` option with [`guild run`](project:/pages/commands/run.md) to print the full command Guild uses when running an operation.

### Environment Variables

Guild makes flag values available as environment variables to each run process. Environment variables are named `FLAG_<em>UPPER_CASE_NAME</em>`{em-code}. *`UPPER_CASE_NAME`* is the flag name coverted to upper case with non-alphanumeric characters converted to undercore characters.

For example, the value for flag *`x`* is read as the environment variable `FLAG_X`.

Use the [`env-name`](project:/pages/reference/guildfile.md#flag-env-name) flag attribute to specify a different environment variable name for a flag.
...
You can alternatively set flag values in a Python global dict variable using `global:<em>VARIABLE_NAME</em>`{em-code} for `flags-dest`.
...
Define flags for an operation using the [`flags`](project:/pages/reference/guildfile.md#operation-flags) operation attribute.
...
Flags can be defined using a value, as shown above, or with a mapping of attributes. If a value is specified, it's used as the default flag value. The default value can otherwise be defined using the [`default`](project:/pages/reference/guildfile.md#flag-default) attribute. The following is equivalent to the configuration above:
...
Flags support a number of attributes for defining settings for help, value checks, type conversion, and interface details. Refer to [*Guild File Reference*](project:/pages/reference/guildfile.md#flag-attributes) for a list of supported attributes.

For more examples of flag definitions, see [*Guild File Cheatsheet*](project:/pages/cheatsheets/guildfile.md#flag-definitions).

## Batch Files

Batch files are files that contain one or more sets of flags to use for a run.

Specify batch files for a run using one or more arguments with the syntax `@<em>PATH</em>`{em-code} where *`PATH`* is the path to a valid batch file.
...
For information on batch file format, see [Batch Files](project:/pages/docs/runs.md#batch-files).
...
A *value list* is a flag value in the format `[<em>VAL1</em>,<em>VAL2</em>,<span data-guild-class="fal fa-ellipsis-h"></span>]`{em-code} where each value is a number, a string, or boolean value.

A value list is processed according to the [*batch operation*](project:/pages/docs/optimization.md) used. The [default batch operation](project:/pages/docs/optimization.md#grid-search) uses values in a grid search. Other batch operations, including [random](project:/pages/docs/optimization.md#random-search) and [sequetial optimizers](project:/pages/docs/optimization.md#sequential-optimization), use the list as a set of choices to select from when suggesting trial values.
...
This command is a sequential optimization using [`gp`](project:/pages/reference/optimizers.md#gp) to minimize *`loss`*. It uses the same flag values. Based on the optimizer, it generates 5 trials using value lists as *choices* to sample from:
...
Sequence functions are specified in the format `<em>NAME</em>[<em>ARGS</em>]`{em-code} where *`NAME`* is one of the functions below and *`ARGS`* is a list of values separated by a colon `:`.
...
Search space functions are specified in the format `<em>NAME</em>[<em>ARGS</em>]`{em-code} where *`NAME`* is one of the functions below and *`ARGS`* is a list of values separated by a colon `:`.
