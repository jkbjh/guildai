...
Use [`guild diff`](project:/pages/commands/diff.md) to compares two runs.
...
For a complete list of options, see [`guild diff`](project:/pages/commands/diff.md).
...
When diffing runs or paths, Guild passes the two [*run directories*](project:/pages/docs/runs.md#run-dir) (or applicable paths if certain options are included) to a diff program. By default, Guild chooses a diff program available on your system based on its capabilities.
...
You can configure the diff program that Guild uses by default in *user configuration*. For more information, see [*User Config Reference*](project:/pages/reference/user-config.md#diff).
