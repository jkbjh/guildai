...
Guild detects patterns in the format: `<em>KEY</em>: <em>VALUE</em>`{em-code}, where *`KEY`* is a scalar identifier that occurs at the start of a line and *`VALUE`* is a valid float represenation. Guild logs these key/value pairs as *scalars*.
