...
Runs are stored under a `runs` directory located in [*Guild home*](/pages/docs/environments#guild-home). To show where Guild saves runs, refer to the `guild_home` attribute shown by [`guild check`](/pages/commands/check). Each run is saved in a unique subdirectory. For more information, see [Run Directory](#run-directory) below.
...
Runs directories are stored together in a `runs` subdirectory of [Guild home](/pages/docs/environments#guild-home).
...
Guild additionally saves run *metadata* in the run directory. This information is shown when you run [`guild runs info`](/pages/commands/runs-info).

### Resource Links

Guild supports *required resources* for runs, which are files that an operation needs to run successfully. For example, a model training operation may require a data set prepared by another operation. These dependencies are defined in a [Guild file](/pages/docs/guildfiles).
...
For for more information on defining and using required resources, see [*Dependencies*](/pages/docs/dependencies).

### Run Metadata

Run metadata consists of:

- [Flags](/pages/docs/flags)
...
Run metadata are stored in files under a Guild-generated `.guild` subdirectory of the run directory. You can list these files for a run using [`guild ls`](/pages/commands/ls) as follows:
...
Start a run using [`guild run`](/pages/commands/run). The `run` command is a multi-feature command that supports the following actions:
...
In [*Get Started*](/pages/start) you [run a mock training script](/pages/start#run-the-script) named `train.py`.
...
For more information on Guild's default behavior, see [*Default Behavior - Python Scripts*](/pages/docs/defaults).

> <span data-guild-class="callout tip">Tip</span> Guild's support for directly script execution is a convenience to get started quickly. Without additional information, Guild makes various [assumptions](/pages/docs/defaults) that may not hold true for your script. Consider using [operations](/pages/docs/operations) for more control over how Guild runs your script. See [Run an Operation](#run-an-operation) below for more information.

#### Auto-Detect Python Script Flags

To run a Python script without additional configuration, Guild inspects the script to determine script [flags](/pages/docs/flags), which provide information to the script.
...
<span data-guild-class="caption">Guild detects use of `argparse` and assumes that command options are used as [*flag*](/pages/docs/flags)</span>
...
Guild logs numeric values as [*scalars*](/pages/docs/scalars) that are written by the script using the format:
...
This behavior can be modified by defining `output-scalars` for an operation in a Guild file. See [Scalars](/pages/docs/scalars) for more information.

### Run an Operation

Run an operation by specifying the operation name. To show the list of available options for a project, use [`guild ops`](/pages/commands/ops). To include installed operation --- i.e. operations defined in installed packaged -- use `-i` with the `ops` command.

With the operation name, start it with [`guild run`](/pages/commands/run). Refer to the command help for options.

### Run a Batch

To run a batch, specify a flag value that implicitly starts a [*batch run*](#batches). Start a batch explicitly by specifying `--optimize` or `--optimizer` with [`guild run`](/pages/commands/run).
...
There are various ways to test an operation. The first way is to simply run it. Consider implementing the [10 Second Rule](/pages/start/use-guild#10-second-rule) for your operations so you can run the full code path in less than 10 seconds.
...
To run an operation on a [remote](/pages/docs/remotes), use the `--remote` option with [`guild run`](/pages/commands/run).

You must define the specified remote in [user configuration](/pages/reference/user-config).

See [*Remotes*](/pages/docs/remotes) for information on defining and using remotes.
...
Commands that support these filters include [`cat`](/pages/commands/cat), [`compare`](/pages/commands/compare), [`export`](/pages/commands/export), [`import`](/pages/commands/import), [`label`](/pages/commands/label), [`ls`](/pages/commands/ls), [`mark`](/pages/commands/mark), [`open`](/pages/commands/open), [`publish`](/pages/commands/publish), [`pull`](/pages/commands/pull), [`push`](/pages/commands/push), [`runs diff`](/pages/commands/runs-diff), [`runs info`](/pages/commands/runs-info), [`runs purge`](/pages/commands/runs-purge), [`runs restore`](/pages/commands/runs-restore), [`runs rm`](/pages/commands/runs-rm), [`runs`](/pages/commands/runs), [`select`](/pages/commands/select), [`tensorboard`](/pages/commands/tensorboard), and [`view`](/pages/commands/view).

### List Runs

List runs using [`guild runs`](/pages/commands/runs). This is an alias for the full command [`guild runs list`](/pages/commands/runs-list).
...
You can filter runs using various filter options. See [`runs list`](/pages/commands/runs-list) for details.

You can list runs in an export archive by specifying the directory with the `-A, --archive` option.

### Get Run Information

Use [`guild runs info`](/pages/commands/runs-info) to show run information. This command outputs a number of run attributes.
...
To list run files, use [`guild ls`](/pages/commands/ls).

To open a file from a local run using an associate program, use [`guild open`](/pages/commands/open).

To open a new command line shell for a run, use `--shell` with [`guild open`](/pages/commands/open). The shell is configured with the run directory as the current directory.

To show the contents of a run file, use [`guild cat`](/pages/commands/cat).
...
|-|-|
| [`compare`](/pages/commands/compare) | Compare run flags and scalars in a tabular view. Command is terminal based works the same when run remotely and locally. See [*Guild Compare*](/pages/docs/compare) for details. |
| [`view`](/pages/commands/view) | Graphical application (web based) for exploring and comparing runs. See [*Guild View*](/pages/docs/view) for details. |
| [`tensorboard`](/pages/docs/tensorboard) | Use TensorBoard to compare run scalars, images, hyperparameters, and other logged summaries. See [*TensorBoard*](/pages/docs/tensorboard) for details.|
| [`runs diff`](/pages/commands/diff) | Use a diff program to compare runs. |

### Delete Runs

Use [`guild runs rm`](/pages/commands/runs-rm) or [`guild runs delete`](/pages/commands/runs-delete) to delete runs. These commands are identical. Use the form you prefer.

By default, when you delete a run, Guild moves the run to a trash directory in case you want to restore it. Use [`guild runs restore`](/pages/commands/runs-restore) to restore deleted runs. List deleted runs using [`guild runs -d`](/pages/commands/runs).

You can permanently delete deleted runs using [`guild runs purge`](/pages/commands/runs-purge). This operation cannot be undone. Purging runs frees disk space.
...
Use [`guild export`](/pages/commands/export) to move or copy runs to a local directory.
...
Use [`guild import`](/pages/commands/import) to move or copy runs from a directory to your environment.

### Copy Runs to and from a Remote System

Use [`guild push`](/pages/commands/push) to copy runs from your local environment to a remote. This is useful for backing up runs to a server or to a cloud service like S3. You can also use this method to share runs with colleagues or deploy models.

Use [`guild pull`](/pages/commands/pull) to copy runs from a remote to your local environment. Use this method to restore archived runs or get runs from colleagues.

## Labels

A *label* is used to describe a run. Run labels are shown in various contexts including runs lists and run comparisons.

To show the label for a specific run, use [`guild runs info`](/pages/commands/runs-info) --- the label is designated by the `label` attribute.
...
By default, Guild generates a label using the [`label`](/pages/reference/guildfile#operation-label) template configured for an operation. If an operation doesn't specify a label template, Guild generates a default label with flag assignments.

Set a different label using the `--label` option with [`guild run`](/pages/commands/run).

You can alternatively *tag* a run with a string that's prepended to the default label. Specify the tag with the `--tag` option.

### Modify a Run Label

Modify the label for one or more runs using [`guild label`](/pages/commands/label). When modifying a label, you have several options.
...
For more information, see [*Hyperparameter Optimization*](/pages/docs/optimization).
