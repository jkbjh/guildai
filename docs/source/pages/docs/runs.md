...
Runs directories are stored together in a `runs` subdirectory of [Guild home](project:/pages/docs/environments.md#guild-home).
...
Guild additionally saves run *metadata* in the run directory. This information is shown when you run [`guild runs info`](project:/pages/commands/runs-info.md).

### Resource Links

Guild supports *required resources* for runs, which are files that an operation needs to run successfully. For example, a model training operation may require a data set prepared by another operation. These dependencies are defined in a [Guild file](project:/pages/docs/guildfiles.md).
...
For for more information on defining and using required resources, see [*Dependencies*](project:/pages/docs/dependencies.md).
...
Run metadata are stored in files under a Guild-generated `.guild` subdirectory of the run directory. You can list these files for a run using [`guild ls`](project:/pages/commands/ls.md) as follows:
...
Start a run using [`guild run`](project:/pages/commands/run.md). The `run` command is a multi-feature command that supports the following actions:
...
In [*Get Started*](project:/pages/start.md) you [run a mock training script](project:/pages/start.md#run-the-script) named `train.py`.
...
To run a Python script without additional configuration, Guild inspects the script to determine script [flags](project:/pages/docs/flags.md), which provide information to the script.
...
<span data-guild-class="caption">Guild detects use of `argparse` and assumes that command options are used as [*flag*](project:/pages/docs/flags.md)</span>
...
Guild logs numeric values as [*scalars*](project:/pages/docs/scalars.md) that are written by the script using the format:
...
This behavior can be modified by defining `output-scalars` for an operation in a Guild file. See [Scalars](project:/pages/docs/scalars.md) for more information.

### Run an Operation

Run an operation by specifying the operation name. To show the list of available options for a project, use [`guild ops`](project:/pages/commands/ops.md). To include installed operation --- i.e. operations defined in installed packaged -- use `-i` with the `ops` command.

With the operation name, start it with [`guild run`](project:/pages/commands/run.md). Refer to the command help for options.

### Run a Batch

To run a batch, specify a flag value that implicitly starts a [*batch run*](#batches). Start a batch explicitly by specifying `--optimize` or `--optimizer` with [`guild run`](project:/pages/commands/run.md).

See [Batches](#batches) below for more information.

## Stage a Run

You can stage a run to start later. To stage a run, use `--stage` with [`guild run`](project:/pages/commands/run.md).

Start a staged run using `\-\-start <em>RUN-ID</em>`{em-code} with `run`.

Staged runs are often used with *queues* to schedule runs. See [*Queues*](project:/pages/docs/queues.md) for more information.
...
Restart a run using `\-\-restart <em>RUN-ID</em>`{em-code} with `run`.
...
To start a new run using an existing run as a prototype, use `\-\-proto <em>RUN-ID</em>`{em-code} with `run`.
...
There are various ways to test an operation. The first way is to simply run it. Consider implementing the [10 Second Rule](project:/pages/start/use-guild.md#10-second-rule) for your operations so you can run the full code path in less than 10 seconds.
...
To run an operation on a [remote](project:/pages/docs/remotes.md), use the `--remote` option with [`guild run`](project:/pages/commands/run.md).

You must define the specified remote in [user configuration](project:/pages/reference/user-config.md).

See [*Remotes*](project:/pages/docs/remotes.md) for information on defining and using remotes.
...
Commands that support these filters include [`cat`](project:/pages/commands/cat.md), [`compare`](project:/pages/commands/compare.md), [`export`](project:/pages/commands/export.md), [`import`](project:/pages/commands/import.md), [`label`](project:/pages/commands/label.md), [`ls`](project:/pages/commands/ls.md), [`mark`](project:/pages/commands/mark.md), [`open`](project:/pages/commands/open.md), [`publish`](project:/pages/commands/publish.md), [`pull`](project:/pages/commands/pull.md), [`push`](project:/pages/commands/push.md), [`runs diff`](project:/pages/commands/runs-diff.md), [`runs info`](project:/pages/commands/runs-info.md), [`runs purge`](project:/pages/commands/runs-purge.md), [`runs restore`](project:/pages/commands/runs-restore.md), [`runs rm`](project:/pages/commands/runs-rm.md), [`runs`](project:/pages/commands/runs.md), [`select`](project:/pages/commands/select.md), [`tensorboard`](project:/pages/commands/tensorboard.md), and [`view`](project:/pages/commands/view.md).

### List Runs

List runs using [`guild runs`](project:/pages/commands/runs.md). This is an alias for the full command [`guild runs list`](project:/pages/commands/runs-list.md).
...
You can filter runs using various filter options. See [`runs list`](project:/pages/commands/runs-list.md) for details.

You can list runs in an export archive by specifying the directory with the `-A, --archive` option.

### Get Run Information

Use [`guild runs info`](project:/pages/commands/runs-info.md) to show run information. This command outputs a number of run attributes.
...
To list run files, use [`guild ls`](project:/pages/commands/ls.md).

To open a file from a local run using an associate program, use [`guild open`](project:/pages/commands/open.md).

To open a new command line shell for a run, use `--shell` with [`guild open`](project:/pages/commands/open.md). The shell is configured with the run directory as the current directory.

To show the contents of a run file, use [`guild cat`](project:/pages/commands/cat.md).
...
|-|-|
| [`compare`](project:/pages/commands/compare.md) | Compare run flags and scalars in a tabular view. Command is terminal based works the same when run remotely and locally. See [*Guild Compare*](project:/pages/docs/compare.md) for details. |
| [`view`](project:/pages/commands/view.md) | Graphical application (web based) for exploring and comparing runs. See [*Guild View*](project:/pages/docs/view.md) for details. |
| [`tensorboard`](project:/pages/docs/tensorboard.md) | Use TensorBoard to compare run scalars, images, hyperparameters, and other logged summaries. See [*TensorBoard*](project:/pages/docs/tensorboard.md) for details.|
| [`runs diff`](project:/pages/commands/diff.md) | Use a diff program to compare runs. |

### Delete Runs

Use [`guild runs rm`](project:/pages/commands/runs-rm.md) or [`guild runs delete`](project:/pages/commands/runs-delete.md) to delete runs. These commands are identical. Use the form you prefer.

By default, when you delete a run, Guild moves the run to a trash directory in case you want to restore it. Use [`guild runs restore`](project:/pages/commands/runs-restore.md) to restore deleted runs. List deleted runs using [`guild runs -d`](project:/pages/commands/runs.md).

You can permanently delete deleted runs using [`guild runs purge`](project:/pages/commands/runs-purge.md). This operation cannot be undone. Purging runs frees disk space.
...
Use [`guild export`](project:/pages/commands/export.md) to move or copy runs to a local directory.
...
Use [`guild import`](project:/pages/commands/import.md) to move or copy runs from a directory to your environment.

### Copy Runs to and from a Remote System

Use [`guild push`](project:/pages/commands/push.md) to copy runs from your local environment to a remote. This is useful for backing up runs to a server or to a cloud service like S3. You can also use this method to share runs with colleagues or deploy models.

Use [`guild pull`](project:/pages/commands/pull.md) to copy runs from a remote to your local environment. Use this method to restore archived runs or get runs from colleagues.

## Labels

A *label* is used to describe a run. Run labels are shown in various contexts including runs lists and run comparisons.

To show the label for a specific run, use [`guild runs info`](project:/pages/commands/runs-info.md) --- the label is designated by the `label` attribute.
...
By default, Guild generates a label using the [`label`](project:/pages/reference/guildfile.md#operation-label) template configured for an operation. If an operation doesn't specify a label template, Guild generates a default label with flag assignments.

Set a different label using the `--label` option with [`guild run`](project:/pages/commands/run.md).

You can alternatively *tag* a run with a string that's prepended to the default label. Specify the tag with the `--tag` option.

### Modify a Run Label

Modify the label for one or more runs using [`guild label`](project:/pages/commands/label.md). When modifying a label, you have several options.
...
For more information, see [*Hyperparameter Optimization*](project:/pages/docs/optimization.md).

### Batch Files

Trials are also generated when you specify a *batch file* for a run. Batch files are specified using `@<em>PATH</em>`{em-code} where *`PATH`* is the path to a file containing trial flags.
