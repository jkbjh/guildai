...
- How the script reads user-provided values, or [*flags*](project:/pages/docs/flags.md)
- How the script communicates numeric results, or [*scalars*](project:/pages/docs/scalars.md), such as training *loss* and *accuracy*

Unless configured otherwise, Guild uses [default rules](project:/pages/docs/defaults.md) to determine this information.

You can configure this information explicitly using a [*Guild file*](project:/pages/docs/guildfiles.md). A Guild file is a human-readable text file named `guild.yml` located in a project directory.
...
| `main` | Guild loads the specified Python module when running the operation. By default, Guild uses the operation name. We could omit `main` in this case but include it here to illustrate its use. For more information, see [*Python Based Operations*](project:/pages/docs/operations.md#python-based-operations). |
| `flags-import` | When a Guild file is used, Guild does not automatically import flags from the main module. You must explicitly import flags or define them for the operation. In this case, we tell Guild to [import all detected flags](project:/pages/docs/flags.md#import-all-flags) from the main module. |
| `output-scalars` | Numeric results like *loss* and *accuracy* are called [*scalars*](project:/pages/docs/scalars.md). Guild detects scalars written as script output (e.g. using `print` or `log` functions in Python) and those that are [logged explicitly](project:/pages/docs/scalars.md#tensorboard-summaries). By default, Guild captures scalars written to output in the format `<em>KEY</em>: <em>VALUE</em>`{em-code}. We could omit `output-scalars` in this case but include it here for illustration. For more information, see [*Output Scalars*](project:/pages/docs/scalars.md#output-scalars). |

</div>

Refer to [*Guild File Reference*](project:/pages/reference/guildfile.md) for details about the Guild file format and available configuration options.
...
Save your changes to `guild.yml` above. Use [`guild operations`](project:/pages/commands/operations.md) to show operations defined for the project:
...
Use [`guild help`](project:/pages/commands/help.md) to show project information:
...
<span data-guild-class="btn next">[Next: Add a Classifier](project:/pages/start/classifier.md)</span>
