...
In the [previous section](project:/pages/start/guildfile.md) you use a Guild file to define a `train` operation. In this section, you enhance the project by adding a real-world classifier.

You cover:

- Multiple [models](project:/pages/docs/guildfiles.md#models) in a single project
- Guild [environments](project:/pages/docs/environments.md) to isolate project work
...
The file defines two models: `sample` and `iris-svm`. The `sample` model provides the `train` operation from the [previous step](project:/pages/start/guildfile.md). The `iris-svm` is a new model, which you add in this section.

> <span data-guild-class="callout highlight">Highlight</span> Use models to group related sets of operations. Models generally correspond to the structures that you train and test with data, though you're free to define models for other purposes. For more information, see [*Models*](project:/pages/docs/guildfiles.md#models).

The Guild file format changes from a mapping of operations, known as [*operation only format*](project:/pages/reference/guildfile.md#operation-only-format), to a list of top-level objects, known as [*full format*](project:/pages/reference/guildfile.md#full-format). Guild supports these two formats to accommodate different requirements. Use *operation only format* when starting to keep your Guild file as simple as possible. Migrate to *full format* as needed to support more configuration.
...
| `flags` | A mapping of flag *name* to flag *definition*. Each flag provides a description, which is used in help text, and a default value. The `kernel` flag defines a list of valid *choices*---one for each supported kernel. For more information, see [*Flags*](project:/pages/docs/flags.md). |
| `output-scalars` | Patterns used to capture [output scalars](project:/pages/docs/scalars.md#output-scalars). In this case, the capture patterns are different from those of `sample:train`. This accommodates the different output generated by the module. |
...
> <span data-guild-class="callout note">Note</span> [`guild ops`](project:/pages/commands/ops.md) is a shortened alias for [`guild operations`](project:/pages/commands/operations.md).
...
In this section, you create a project-specific *environment* to keep project libraries and runs separate from other projects. For more information, see [*Environments*](project:/pages/docs/environments.md).
...
Having adding `requirements.txt` to the project, initialize a new environment in the project using [`guild init`](project:/pages/commands/init.md):
...
> <span data-guild-class="callout note">Note</span> You can always use the traditional method of activating a virtual environment created with [`guild init`](project:/pages/commands/init.md). Guild environments are standard Python virtual environments.
...
Use [`guild check`](project:/pages/commands/check.md) to confirm that Guild uses the activated environment:
...
Use [`guild compare`](project:/pages/commands/compare.md) to compare results:
...
>- Evaluating results in [Guild View](project:/pages/docs/view.md), [Guild Compare](project:/pages/docs/compare.md), and [TensorBoard](project:/pages/docs/tensorboard.md)

Now that you're familiar with Guild, try using it for your own work.

<span data-guild-class="next btn">[Use Guild in a Project](project:/pages/start/use-guild.md)</span>
