...
In the [previous section](/pages/start/guildfile) you use a Guild file to define a `train` operation. In this section, you enhance the project by adding a real-world classifier.

You cover:

- Multiple [models](/pages/docs/guildfiles#models) in a single project
- Guild [environments](/pages/docs/environments) to isolate project work
...
The file defines two models: `sample` and `iris-svm`. The `sample` model provides the `train` operation from the [previous step](/pages/start/guildfile). The `iris-svm` is a new model, which you add in this section.

> <span data-guild-class="callout highlight">Highlight</span> Use models to group related sets of operations. Models generally correspond to the structures that you train and test with data, though you're free to define models for other purposes. For more information, see [*Models*](/pages/docs/guildfiles#models).

The Guild file format changes from a mapping of operations, known as [*operation only format*](/pages/reference/guildfile#operation-only-format), to a list of top-level objects, known as [*full format*](/pages/reference/guildfile#full-format). Guild supports these two formats to accommodate different requirements. Use *operation only format* when starting to keep your Guild file as simple as possible. Migrate to *full format* as needed to support more configuration.
...
| `flags` | A mapping of flag *name* to flag *definition*. Each flag provides a description, which is used in help text, and a default value. The `kernel` flag defines a list of valid *choices*---one for each supported kernel. For more information, see [*Flags*](/pages/docs/flags). |
| `output-scalars` | Patterns used to capture [output scalars](/pages/docs/scalars#output-scalars). In this case, the capture patterns are different from those of `sample:train`. This accommodates the different output generated by the module. |
...
> <span data-guild-class="callout note">Note</span> [`guild ops`](/pages/commands/ops) is a shortened alias for [`guild operations`](/pages/commands/operations).
...
In this section, you create a project-specific *environment* to keep project libraries and runs separate from other projects. For more information, see [*Environments*](/pages/docs/environments).
...
Having adding `requirements.txt` to the project, initialize a new environment in the project using [`guild init`](/pages/commands/init):
...
> <span data-guild-class="callout note">Note</span> You can always use the traditional method of activating a virtual environment created with [`guild init`](/pages/commands/init). Guild environments are standard Python virtual environments.
...
Use [`guild check`](/pages/commands/check) to confirm that Guild uses the activated environment:
...
Use [`guild compare`](/pages/commands/compare) to compare results:
...
>
>- [Runs](/pages/docs/runs)
>- [Operations](/pages/docs/operations)
>- [Hyperparameter optimization](/pages/docs/optimization)
>- [Environments](/pages/docs/environments)
>- Evaluating results in [Guild View](/pages/docs/view), [Guild Compare](/pages/docs/compare), and [TensorBoard](/pages/docs/tensorboard)

Now that you're familiar with Guild, try using it for your own work.

<span data-guild-class="next btn">[Use Guild in a Project](/pages/start/use-guild)</span>
