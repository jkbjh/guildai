...
Show the runs with [`guild runs`](/pages/commands/runs):
...
Runs 1 through 3 are [*trial runs*](/pages/docs/runs#trial-runs) generated by the command. Run 4 is a [*batch run*](/pages/docs/runs#batches). Batch runs generate trials and are denoted using `+` in their name.

### Compare Runs

Compare *`loss`* across runs using [`guild compare`](/pages/commands/compare):
...
> <span data-guild-class="callout highlight">Highlight</span> [Guild Compare](/pages/docs/compare) is a convenient way to review run results. It works across remote terminal connections as well as local environments and doesn't require a web browser or port configuration.
...
Guild integrates [TensorBoard](https://www.tensorflow.org/tensorboard) for exploring runs. Use  [`guild tensorboard`](/pages/commands/tensorboard) to view current runs:
...
By default, Guild attempts to minimize *`loss`* as an objective. The `train.py` script logs *`loss`* as a [*scalar*](/pages/docs/scalars). If the script uses a different objective, specify the scalar using `--minimize` or `--maximize` options with [`guild run`](/pages/commands/run).

Run 10 trials using the [`gp`](/pages/reference/optimizers#gp) optimizer, which uses Bayesian optimization with Gaussian processes, and search over values of *`x`* between `-2.0` and `2.0`:
...
For list of available optimizes, see [*Optimizers Reference*](/pages/reference/optimizers).
...
To restart a run, specify `--restart` with [`guild run`](/pages/commands/run).
...
The option `-Fo random-starts=0` is an [*optimizer flag*](/pages/docs/runs#optimizer-flags) that sets the number of random starts to `0`.

### Evaluate Bayesian Optimization Results

Run [`guild compare`](/pages/commands/compare) for the last 20 runs to evaluate the results of the Bayesian optimization:
...
You can also use [TensorBoard](/pages/docs/tensorboard) to evaluate optimization results.
...
> - Use [Guild Compare](/pages/docs/compare) and [TensorBoard](/pages/docs/tensorboard) to study optimization trials to help find the best hyperparameters for your models.

In the next section, you learn how to manage runs.

<span data-guild-class="next btn">[Next: Manage Runs](/pages/start/runs)</span>
