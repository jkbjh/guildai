...
In the [previous section](project:/pages/start.md) you use Guild to run `train.py` --- a sample training script. In this section, you run the same script with different hyperparameter values to find lower values of *`loss`*. This process is known as [*hyperparameter optimization*](https://en.wikipedia.org/wiki/Hyperparameter_optimization), or hyperparameter *tuning*.
...
Guild lets you run multiple trials in a batch by specifying them as a list in the form `[<em>VAL1</em>,<em>VAL2</em>,...,<em>VALN</em>]`{em-code}.
...
Show the runs with [`guild runs`](project:/pages/commands/runs.md):
...
Runs 1 through 3 are [*trial runs*](project:/pages/docs/runs.md#trial-runs) generated by the command. Run 4 is a [*batch run*](project:/pages/docs/runs.md#batches). Batch runs generate trials and are denoted using `+` in their name.

### Compare Runs

Compare *`loss`* across runs using [`guild compare`](project:/pages/commands/compare.md):
...
> <span data-guild-class="callout highlight">Highlight</span> [Guild Compare](project:/pages/docs/compare.md) is a convenient way to review run results. It works across remote terminal connections as well as local environments and doesn't require a web browser or port configuration.
...
Guild integrates [TensorBoard](https://www.tensorflow.org/tensorboard) for exploring runs. Use  [`guild tensorboard`](project:/pages/commands/tensorboard.md) to view current runs:
...
Grid search is a systematic search across a subset of hyperparameter space. Guild supports a special  [*sequence function*](project:/pages/docs/flags.md#sequence-functions) syntax for specifying value ranges. Ranges are specified using the format `<em>FUNCTION</em>[<em>START</em>:<em>END</em>:<em>STEP_OR_COUNT</em>]`{em-code} where *`FUNCTION`* is the type of sequence and *`START`* and *`END`* mark the start and end of the sequence respectively. *`STEP_OR_COUNT`* is the range step or value count depending on the function used.

Use the [`linspace`](project:/pages/docs/flags.md#linspace) flag function to run four trials where *`x`* is evenly spaced between `-0.6` and `0.6`:
...
Search space is specified by special [*search space functions*](project:/pages/docs/flags.md#search-space-functions), which include [`uniform`](project:/pages/docs/flags.md#uniform) and [`loguniform`](project:/pages/docs/flags.md#loguniform). The `uniform` function name may be omitted.

To search over a uniformly distributed range of values, specify a flag value in the format `[<em>MIN</em>:<em>MAX</em>]`{em-code}. By default, Guild runs 20 trials using randomly chosen values within the specified range. Use `--max-trials` to specify a different limit.
...
By default, Guild attempts to minimize *`loss`* as an objective. The `train.py` script logs *`loss`* as a [*scalar*](project:/pages/docs/scalars.md). If the script uses a different objective, specify the scalar using `--minimize` or `--maximize` options with [`guild run`](project:/pages/commands/run.md).

Run 10 trials using the [`gp`](project:/pages/reference/optimizers.md#gp) optimizer, which uses Bayesian optimization with Gaussian processes, and search over values of *`x`* between `-2.0` and `2.0`:
...
For list of available optimizes, see [*Optimizers Reference*](project:/pages/reference/optimizers.md).
...
To restart a run, specify `--restart` with [`guild run`](project:/pages/commands/run.md).
...
The option `-Fo random-starts=0` is an [*optimizer flag*](project:/pages/docs/runs.md#optimizer-flags) that sets the number of random starts to `0`.

### Evaluate Bayesian Optimization Results

Run [`guild compare`](project:/pages/commands/compare.md) for the last 20 runs to evaluate the results of the Bayesian optimization:
...
You can also use [TensorBoard](project:/pages/docs/tensorboard.md) to evaluate optimization results.
...
> - Use [Guild Compare](project:/pages/docs/compare.md) and [TensorBoard](project:/pages/docs/tensorboard.md) to study optimization trials to help find the best hyperparameters for your models.

In the next section, you learn how to manage runs.

<span data-guild-class="next btn">[Next: Manage Runs](project:/pages/start/runs.md)</span>
