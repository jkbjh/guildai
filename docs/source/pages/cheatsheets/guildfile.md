...
Auto-detect [flags interface](project:/pages/docs/flags.md#flags-interface):
...
Use arg name to set nested values with `global:<em>NAME</em>`{em-code} interface:
...
Named resource (requires [full format](project:/pages/reference/guildfile.md#full-format) Guild file):
...
Define default optimizer (applies when `--optimize` used with [`run`](project:/pages/commands/run.md) command):
